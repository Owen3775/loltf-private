// Source made by Fooglox#6035
// Release Date: 02.05.2023 
// Updated to Latest Patch
// If you need help dm me -> Fooglox#6035  /  discord.gg/officialcc
...
{
	std::string opt;
	std::cout << "Do you want to load The cheat driver? (y/n): ";
	std::cin >> opt;
...
int main()
{
	...
	Log3(skCrypt(" "));
	printf("[ LOLTF ] Fortnites Id Base Is: %p", (void*)base_address);
	Sleep(1500);
	...
	// -------------------------------------------------------
	   ...
	   for (int i = 0; i < PlayerSize; i++)
	   {
		   ...
		   if (MyTeamId != ActorTeamId)
		   {
			   if (distance <= distanceThreshold)
			   {
				   ...
				   if (InstaRes) {
					   if (CurrentWeapon)
					   {
						   write<bool>(CurrentWeapon + 0x1061, true); //AFortWeapon	bIgnoreTryToFireSlotCooldownRestriction	0xf41	bool
					   }
				   }
		   ...
		   if (norecoil)
		   {
			   if (Actors::LocalPawn)
			   {
				   uintptr_t LocalMesh = read<uintptr_t>(Actors::LocalPawn + Offsets::External::Mesh);
				   if (LocalMesh)
				   {
					   write<int>(LocalMesh + 0xA6A, 1); //USkeletalMeshComponent	bForceRefpose : 1	0x9aa	char
				   }
			   }
			   //write<float>(Actors::PlayerController + 0x64, -1);
		   }
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin("                                     Fortnite External by Fooglox#6035 @Fortnite", NULL, 34);
		static POINT Mouse;
		GetCursorPos(&Mouse);
